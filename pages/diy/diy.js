...
Page({
    ...
    // 初始化获取分类数据
    initGetGoods: function(_this, resolve, reject) {
        var uid = swan.getStorageSync('uid')
        var login_sign = swan.getStorageSync('sign')

        uid = uid ? uid : 0
        login_sign = login_sign ? login_sign : initSign
        swan.request({
    ...
    // 初始化diy产品数据
    initGetList: function(_this, resolve, reject) {
        var uid = swan.getStorageSync('uid')
        var login_sign = swan.getStorageSync('sign')
    ...
    init: function(_this) {
        ...
        let promise1 = new Promise((resolve, reject) => {
            _this.initGetGoods(_this, resolve, reject)
        })
        promise1.then((data) => {
            if (data == 'success') {
                // 获取产品列表数据
                return new Promise((resolve, reject) => {
                    _this.initGetList(_this, resolve, reject)
                })
            }
        }).then((data) => {
            swan.hideLoading()
        ...
        swan.getStorage({
            ...
            success: function(res) {
                if (res.data) {
                    ...
                    var currentAlias = _this.data.currentAlias
                    if (currentAlias != 'chassis') {
