...
Page({
    ...
    onLoad: function (options) {
        ...
        if (goods_name) {
           this.data.paramers.goods_name = goods_name 
        }
    },
    // 页面加载完毕获取首页数据
    onShow: function() {
        ...
        if (shop_id == 1 || shop_id == '') {
            ...
            var goods_name = this.data.paramers.goods_name
            if (goods_name) {
    ...
    onReachBottom: function() {
        ...
        swan.request({
            ...
            fail: function(res) {
                console.log(res);
                swan.navigateTo({
                    url: '/pages/goodsDetail/goodsDetail?id=' + id + '&shop_id=' + shop_id
                })
            }
        })
    },
    linkToDiy: function(event) {
        ...
        var diy = event.currentTarget.dataset.diy
        if (diy == 1) {
            // 官方自营跳转到diy
            var type = 'host'
